...
import { completeInvitation } from '../../lib/adminDataService';
...
/**
 * Página pública para aceptar una invitación de equipo (ver
 * `POST /api/users/invite`). Comparte casi toda su lógica con
 * `src/app/reset-password/page.tsx`: el enlace de invitación de Supabase Auth
 * usa el mismo mecanismo PKCE que un enlace de recuperación de contraseña, y
 * dispara el mismo evento `SIGNED_IN` al aceptarse. La diferencia real es que
 * aquí, al establecer la contraseña, además se llama a `completeInvitation()`
 * para pasar el perfil de `pending` a `active`.
 */
export default function AcceptInvitationPage() {
      ...
      try {
        if (typeof window !== 'undefined') {
          ...
          if (code) {
            ...
            if (exchangeError) {
              console.warn('Error intercambiando código de invitación:', exchangeError);
            }
          }
        }
  ...
  const handleActivateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error al activar la cuenta invitada:', err);
      setErrorMessage(
        err.message || 'No fue posible activar tu cuenta. El enlace puede haber expirado o ya haber sido usado.'
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative w-full max-w-md bg-white border border-[#dadce0] rounded-2xl p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col items-center text-center pb-5 border-b border-[#f1f3f4]">
          ...
          <h1 className="text-xl font-semibold text-[#1f1f1f] tracking-tight">
            Bienvenido a Valentina AI
          </h1>
          <p className="text-xs text-[#5f6368] mt-1">
            Activa tu cuenta en la Consola Empresarial
          </p>
        </div>
          ...
          {checkingSession ? (
            ...
            <div className="p-5 rounded-2xl bg-[#e6f4ea] border border-[#ceead6] text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-[#5f6368] leading-relaxed">
                  Tu contraseña ha sido registrada de forma segura. Ya puedes ingresar a la plataforma.
                </p>
              </div>
            ...
            <div className="p-5 rounded-2xl bg-[#fef7f0] border border-[#feeedb] text-center space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-[#5f6368] leading-relaxed">
                  Esta invitación ya fue utilizada o caducó por razones de ciberseguridad. Pide a tu administrador que
                  te reenvíe la invitación desde el Directorio de Credenciales.
                </p>
              </div>
            ...
            <form onSubmit={handleActivateAccount} className="space-y-4">
              <div className="flex items-center gap-2 p-3 rounded-xl bg-[#f0f4f9] border border-[#d3e3fd] text-xs text-[#0b57d0]">
                ...
                <span>Crea tu contraseña de acceso. Nadie más la conocerá.</span>
              </div>
